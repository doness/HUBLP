...
<nav class="navbar" role="navigation" aria-label="Main navigation">
    <div class="nav-container">
        <a href="index.html" class="logo" style="cursor: pointer; text-decoration: none;">
        ...
        <div class="nav-links" id="navLinks">
            <a href="index.html#features" class="flux-btn flux-btn-ghost flux-btn-sm" data-i18n="nav-features" onclick="event.preventDefault(); window.smoothScroll('#features', 1500); window.closeMobileMenu();">Features</a>
            <a href="index.html#testimonials" class="flux-btn flux-btn-ghost flux-btn-sm" data-i18n="nav-testimonials" onclick="event.preventDefault(); window.smoothScroll('#testimonials', 1500); window.closeMobileMenu();">Testimonials</a>
            <a href="index.html#pricing" class="flux-btn flux-btn-ghost flux-btn-sm" data-i18n="nav-pricing" onclick="event.preventDefault(); window.smoothScroll('#pricing', 1500); window.closeMobileMenu();">Pricing</a>
            <a href="index.html#faq" class="flux-btn flux-btn-ghost flux-btn-sm" data-i18n="nav-faq" onclick="event.preventDefault(); window.smoothScroll('#faq', 1500); window.closeMobileMenu();">FAQ</a>
            
            <div class="lang-dropdown">
                <button class="lang-btn" onclick="toggleLangDropdown()" aria-label="Select language" style="position: relative;">
            ...
            <a href="https://nexus.aika.agency/login" class="flux-btn flux-btn-ghost flux-btn-sm" data-i18n="nav-login" rel="noopener">Client Login</a>
            <a href="https://nexus.aika.agency/signup" class="flux-btn flux-btn-primary flux-btn-sm" data-i18n="nav-try" rel="noopener">Try Free</a>
        </div>
    </div>
</nav>
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    initTranslations(); // Now synchronous again
    initTranslations(); // Now synchronous again
    ...
    try {
        ...
        if (headerContainer) {
            try {
                ...
                console.warn('Using fallback header.', fetchErr);
        ...
        if (footerContainer) {
            try {
                ...
                console.warn('Using fallback footer.', fetchErr);
                footerContainer.innerHTML = FALLBACK_FOOTER;
            }
        }
    } catch (e) {
        console.error("Critical error in loadComponents", e);
    }
}
